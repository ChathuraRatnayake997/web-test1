...
const defaultSections = ['hero.json', 'about.json', 'education.json', 'publications.json', 'projects.json', 'bulbul.json', 'contact.json'];

Promise.all([fetchJSON('data/site.json'), ...defaultSections.map(f => fetchJSON('data/' + f))])
  .then(([siteData, ...sectionData]) => {
    const map = {};
    defaultSections.forEach((f, i) => { map[f] = sectionData[i]; });
    const files = siteData?.sections || defaultSections;
    const sections = files.map(f => map[f]).filter(Boolean);
